...
const colorClassName = (color?: StateStatus) => {
  switch (color) {
    case StateStatus.WARNING:
      return styles['bg-warning']
    case StateStatus.DANGER:
      return styles['bg-danger']
    default:
      return styles.noBg
  }
}
...
const renderRows: <Item>(
  ...
  __: Column<Item>[],
  ___: boolean,
) => JSX.Element[] = (items, columns, showRowNumber: boolean) =>
  items.map((item, i) => renderRow(item, columns, i, showRowNumber))
...
const renderRow: <Item>(
  ...
  index: number,
  showRowNumber: boolean,
) => JSX.Element = (item, columns, index, showRowNumber: boolean) => {
  return (
    <tr key={index}>
...
type Column<Item> = {
  ...
  background?: (item: Item) => StateStatus
  alignment?: Alignment
}
...
export const Table: <Item>(props: Props<Item>) => JSX.Element = ({
  ...
  return (
    <table className={styles.table}>
      ...
      <tbody>{renderRows(sortedData, columns, showRowNumber ?? false)}</tbody>
    </table>
  )
}

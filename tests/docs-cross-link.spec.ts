...
// breakdown card must point to /docs (or /expert-docs) with an anchor
// fragment. The existing docs-deep.spec.ts covers anchor scrolling once
// you're already at /docs#xxx, but nothing ties the source card → target
// URL together.
//
// The Guide ↗ link opens in a new tab (target=_blank), so we can't follow
// it — we assert the href shape instead.
...
async function openSheet(page: Page, route: '/test-' | '/test-?level=expert') {
  await page.goto(`${route.startsWith('/test-?') ? route + '&' : route + '?'}v=${V01}`)
...
test.describe('docs cross-links — guide link hrefs from detail sheet', () => {
  test('Overview / APY-composition card Guide ↗ points to /docs#... in basic mode', async ({
    page,
  }) => {
    await openSheet(page, '/test-')
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `link #${i} href`).toMatch(/^\/(expert-)?docs#[a-z0-9-]+$/)
      // /test- runs under UserLevel.Basic → href should be /docs#...

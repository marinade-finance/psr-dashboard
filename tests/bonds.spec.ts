...
test.describe('Bonds basic', () => {
  ...
  test('coverage stats chips visible (Bonds funded, Total bonds, Total stake)', async ({
    page,
  }) => {
  ...
  test('marinade stake column sorted descending by default (first ≥ second)', async ({
    page,
  }) => {
    ...
    const parseVal = (s: string) =>
      parseFloat(s.replace(/[^0-9.,]/g, '').replace(/,/g, '')) || 0
  ...
  test('validator column shows truncated vote accounts', async ({ page }) => {
    ...
    // Validator column shows truncated pubkeys like "XXXXXXXX…XXXX" (uses
    // a single ellipsis char, not three dots — see ValidatorIdentity).

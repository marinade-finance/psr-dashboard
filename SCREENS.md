...
| Next Step | `nextStep`    | One-line tip from `getValidatorTip`, pill capped at `max-w-[260px]` so the column stays rhythmic. **Colour = severity** via `getTipStyle(tip.urgency)`. **Glyph = lever** via `getTipIcon`: `TipConstraint.BOND` → shield glyph, `BID`/`RANK` → ascending-chevrons glyph (same lever — raise the bid), `CAP` → cap glyph; all non-directional and severity-agnostic. Only `TipConstraint.NONE` is directional and keyed off the real signed delta — ↗ gain, ↘ loss, → at target — so an up-arrow can never appear on a losing or blocked row. One escalation: `tip.alert === true` (an estimated bond risk fee this epoch) swaps the glyph for the octagon `ICON_ALERT`. The muted "Bid below winning price." label is set by the engine (`tip.chip`, from `bidCta`), never re-derived in the table: it fires for the calm, out-of-set-below-winning row — the contiguous below-cutoff block whose bid genuinely trails the winning price; an expected state, not an alarm, with the full sentence (`Raise bid to qualify for stake.`) in the detail panel. It mutes the pill only — the row tint is set independently by membership (`!inSet` → `bg-muted/40`). A _defending_ out-of-set row (losing meaningful stake, `isDefending`) drops the chip so its pill keeps the warning-yellow colour and the full CTA, standing out inside the same muted row. In-set rank tips (a winning validator below the redelegation priority frontier) also carry no chip — they keep one unified text, `Raise bid to grow stake next epoch.` (`RAISE_TO_GROW` in `tip-engine.ts`, fired from two different delta states so it can't drift into two different-sounding CTAs for the same situation) — and normal severity colour, since their bid is above winning, not below. Out-of-set validators whose `revShare.totalPmpe` already clears the winning total surface the binding reason directly (`outOfSetCta`, gated by `outOfSetGate`): `Blocked from SAM this epoch.`, `Blacklisted — N penalty this epoch.` (or plain `Blacklisted.` when no penalty charges), `Not eligible — check client version and vote credits.`, or a binding-cap cause line (`N at country cap` / `N at ASO cap` / `At per-validator cap` / `At your bond cap` / `At the risk cap` / `At a concentration cap`) suffixed `.`; any other case returns null and `deltaCta` owns the message. **Cause beats symptom:** for such a row `getValidatorTip` drops `deltaCta` from the candidate set entirely whenever any lever CTA survives, so a calm low-bond row reads `Top up bond to N SOL to grow stake.` and never `Losing N SOL next epoch.` — the loss IS the gate's effect, and the gate is the actionable half. Severity is per-branch: samBlocked and an active blacklist penalty are critical-red (alert glyph); the rest read warning when meaningful stake is leaving (`isDefending`), else info. `outOfSetGate` skips `bondBelowMin` in the CTA (`bondCta` owns that lever) but still reports it as the badge caption. In-set `deltaCta` says ``At your `maxStakeWanted` setting.`` only when the validator's own `maxStakeWanted` is what actually binds — it is `> 0`, sits at/above the auction floor `max(minMaxStakeWanted, active)`, and the target reaches it. A setting under that floor is silently clipped up to it (SDK `buildSamWantConstraints`), so the shown cap is not what binds and the target can exceed it — those rows fall through to `Raise bid to grow stake next epoch.` (bid below the priority frontier) or `At target stake.` (frontier already cleared). Otherwise `At target stake.` for zero / cap-bound deltas. |
...
| Out of set                  | `bg-muted/40`, hover `bg-muted` — membership is tone-neutral, never destructive; severity lives in the Next Step pill   |
...
The Bond chip stays red for any below-minimum bond — `bondHealthFromAuction` returns `critical` whenever `bondBalanceSol < minBondBalanceSol`, fee pending or not. It is the tip that softens: `bondAdvice`/`bondCta` downgrade to `TipUrgency.NEUTRAL` + grey tone when no fee is charged (eligibility, not urgency: "Top up bond to X to qualify."). Both `no-bond` and `critical` resolve to the same red chip via the shared `DESTRUCTIVE_CHIP` style; only the label differs. `tipBannerSeverity` (in `breakdowns/card.tsx`) routes the bond-NEUTRAL case to the `grey` banner tone so the header tip banner agrees with the Next Step pill.
...
- **Rank glyph** — large mono `#N` (always positive), coloured by tip
  ...
  Next Step uses. Sub-label below it reads `out of set` when unranked,
  else `at winning edge` / `N places above winning` / `N places below
winning` off `values.cutoffRank`.
- Validator display name + truncated vote account.
- **In Set / Out of Set** pill — primary-light when in set, muted when
  out. Membership is not severity, so the out-of-set pill is grey, not
  destructive. Always carries a tooltip: `outOfSetGateLabel(gate)` when
  `outOfSetGate` identifies a gate, else a one-line explanation of the
  membership state.
...
`tipBannerSeverity(tip, bondHealth)`: bond tips colour off bond-health
...
- **Concentration** card (conditional — hidden when the validator is
  ...
  of TOTAL NETWORK stake — every validator sharing this one's country /
  ASO, summed as `externalActivatedSol + marinadeSamTargetSol`, over
...
- **APY Composition** — `ApyCompositionCard`. Segmented bar showing
  ...
  `Cleared — {gate}`, where the gate comes from `outOfSetGateLabel`
  (`Bond below 5.0 SOL minimum`, `Germany at country cap`, `At your bond
cap`, `At the risk cap`, `Blacklisted`, …) — the same source the
  `Out of Set` pill tooltip reads, so the two cannot name different
  causes. `Cleared, but out of set` remains the fallback for the rows
  where `outOfSetGate` returns null. No bid clears a cap or a block, so
  neither variant is clickable. Destructive gets `Fix in Bidding ↗`, and
  the whole stack becomes one button that switches the panel to the
...
Whenever the PSR estimate query has no data to show, the PSR section
renders instead as its `SectionHeader` plus a single `Estimate for this
epoch` row, and the total is relabelled `Total payment — excluding PSR`.
Two states reach it: `psrEstimatesLoading` (the fetch is in flight —
row reads `loading…`) and `psrEstimatesUnavailable` (an error with
nothing cached — row reads `unavailable`). A failed refetch that still
holds good estimates is NEITHER: the estimates render normally and the
total stays `Total payment`.

Status banner summarises the combined state from `baseStatus`, four
variants: green `You will pay X SOL in total this epoch — no
penalties.`, red `You will pay X SOL in total this epoch — including
Y SOL in penalties.`, neutral `You will pay at least X SOL this epoch
— PSR settlement estimates are still loading, so this total is
incomplete.` while `psrEstimatesLoading`, or amber `You will pay at
least X SOL this epoch — PSR settlement estimates could not be loaded,
so this total is incomplete.` when `psrEstimatesUnavailable`. The right-side action pill

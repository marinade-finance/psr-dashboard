...
Below-minimum bond reads red ONLY when a bond risk fee is pending — `bondHealthFromAuction` returns `critical` when `bondBalanceSol < minBondBalanceSol`, but `bondAdvice`/`bondCta` downgrade to `TipUrgency.NEUTRAL` + grey tone when no fee is charged (eligibility, not urgency: "Top up bond to X to qualify."). A validator with no bond at all is `no-bond` ("No bond") — distinct from `soft` ("Adequate"), which it was previously mislabelled as. Both `no-bond` and `critical` resolve to the same red chip via the shared `DESTRUCTIVE_CHIP` style; only the label differs. `tipBannerTone` (in `breakdowns/card.tsx`) routes the bond-NEUTRAL case to the new `grey` banner tone so the header tip banner agrees with the pill colour.

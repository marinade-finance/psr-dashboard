...
for (const [name, url] of Object.entries(SPECS)) {
  ...
  await Bun.write(`${OUT_DIR}/${name}.ts`, src.replace(/^export const api = new Zodios\(endpoints\)\n\n/m, ''))
}
...
function hoistDefs(doc: Record<string, unknown>): Record<string, unknown> {
  ...
  walked.components = { ...components, schemas: { ...schemas, ...(components.schemas as object ?? {}) } }
  return JSON.parse(JSON.stringify(walked).replace(/"#\/\$defs\/([^"]+)"/g, '"#/components/schemas/$1"'))
}
...
function walk(v: unknown, schemas: Record<string, unknown>): unknown {
  ...
  if (v !== null && typeof v === 'object') {
    ...
    if ('$defs' in o) { Object.assign(schemas, o['$defs']); delete o['$defs'] }
    return Object.fromEntries(Object.entries(o).map(([k, val]) => [k, walk(val, schemas)]))
  }
  return v
}
...
function dedupeOperationIds(doc: Record<string, unknown>) {
  ...
  for (const methods of Object.values((doc.paths ?? {}) as Record<string, Record<string, unknown>>)) {

...
| `['epoch-meter']`                      | `EpochMeter` (nav, all pages)                                                          | Lean scalars (network epoch, settlement epochs, live-epoch start) via `fetchEpochMeterData`, which reuses the shared `['protected-events']` cache but retains only the scalars — so the always-mounted nav never pins the full payload. `staleTime` 5 min, refetch hourly. |

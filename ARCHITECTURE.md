...
- **Shared utilities**: `@marinade.finance/ts-common` 5.1.2 — owns the APY
  ...
  with `.toNumber()` at the UI boundary.
...
| File                   | What it does                                                                                                                                                                                                                                                       |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `sam.ts`               | Loads and runs the SDK auction (`loadSam`). Augments results with per-validator expected stake change. Selectors for APY, stake, bid, budget, priority frontier. `selectEpochDurationSeconds` derives the annualisation basis from the auction's own `slotParams`. |
| `simulation.ts`        | Builds `SourceDataOverrides` from form edits; produces ghost-row state for the simulation UI.                                                                                                                                                                      |
| `calculations.ts`      | Re-exports `blockRewardsSharedFrac` and bond-gauge geometry (`bondGaugeScaleMax`, `bondGaugeCriticalFrac`) from ds-sam-calc. APY compounding now lives in ts-common.                                                                                               |
...
| `protected-events.ts`               | Fetches on-chain protected events from `/v1/protected-events`; humanises reason strings; owns the settled-epoch selectors (`selectLatestProcessedEpoch`, `selectUnsettledEstimates`, `selectCurrentEpochEstimates`) shared by the Events page and the Payments tab. `/v1` returns both bond configs and all products, so `selectSamBiddingSettlements` allowlists exactly `(bond_type: bidding, product: sam)` and drops a fetched row missing either — this dashboard is the SAM view. Carries an override layer over the generated schema (optional `bond_type` / `product`, lenient `reason`) so one anomalous row cannot reject the whole array. |
| `protected-events-estimator.ts`     | Derives low-credits / commission-increase estimates for unsettled epochs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `epoch.ts`                          | Epoch-meter logic: network epoch, settlement status, chip / tooltip copy. Also `fetchEpochInfo` (Solana RPC `getEpochInfo`, slot-accurate progress) and `fetchSlotDurationSeconds` (`getRecentPerformanceSamples`, measured seconds per slot); both best-effort, null on failure.                                                                                                                                                                                                                                                                                                                                                                    |
...
| `constants.ts`   | `LAST_DRYRUN_EPOCH`; re-exports `LAMPORTS_PER_SOL` and `pmpeToSol`.                                                                                         |
...
AggregatedValidator[]`, `stakeAmounts`, `rewards`, `slotParams`,
`blacklist`; after `transformValidators()` validators gain eligibility
fields (`samEligible`, `samBlocked`).

`slotParams.slotsPerYear` is agave's nominal for the slot-time regime the
inputs were normalised to. The SDK normalises its inflation window to that
same nominal, so the dashboard annualises against
`epochDurationSecondsFromSlotsPerYear(slotsPerYear)` — never a measured
multi-epoch average, which would count the SIMD-0525 slot-time step twice.
This value is threaded as `epochDurationSeconds` from `SamPage` into
`SamTable` and `ValidatorDetail`.
...
| `['epoch-info']`                       | `EpochMeter`                                                                           | Two parallel Solana RPC calls under one key — `getEpochInfo` (slot-accurate progress) and `getRecentPerformanceSamples` (measured seconds per slot, since SIMD-0525 makes any nominal slot time wrong); `staleTime` / `refetchInterval` 10 min. Sole progress source — when `getEpochInfo` does not resolve the meter shows `RPC unavailable` rather than estimating from timestamps. If only the slot rate is missing, the gauge still renders exactly (it is a ratio of slot counts) and the `~Nh remaining` countdown is dropped. A rate outside 0.05–2 s/slot counts as missing: a partially stalled cluster reports well-formed windows carrying a trickle of slots, and pooling cannot recover a rate from them. |

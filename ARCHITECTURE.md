...
| `protected-events.ts`               | Fetches on-chain protected events from `/v1/protected-events`; humanises reason strings; owns the settled-epoch selectors (`selectLatestProcessedEpoch`, `selectUnsettledEstimates`, `selectCurrentEpochEstimates`) shared by the Events page and the Payments tab. `/v1` returns both bond configs and all products, so `selectSamBiddingEvents` allowlists `(bond_type: bidding, product: sam)` — this dashboard is the SAM view. Carries an override layer over the generated schema (optional `bond_type` / `product`, lenient `reason`), same rationale as `bonds.ts`. |

...
| `['cluster-stats']`                    | `ConcentrationPage`                                                                    | Network-wide ASO and country shares for the comparison column on both splits. Deliberately never gates render: the page shows the SAM split even if this does not resolve, and a group with no network entry reads `—`, not `0%`. `staleTime: 5 min` default. |

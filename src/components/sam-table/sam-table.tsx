...
const GHOST_ROW =
  'ghostRow !cursor-default pointer-events-none [&_td]:line-through [&_td]:text-slate-400 [&_td]:!bg-[rgba(80,70,100,0.35)] [&_td_span]:line-through [&_td_div]:line-through'
...
function bondDotClass(color: Color): string {
  return bondDotColor[color] ?? 'bg-slate-400'
}
...
function renderEditableCell(
  ...
  return (
    <div className="relative inline-block">
      ...
      <input
        type="number"
        className="absolute right-0 top-1/2 -translate-y-1/2 w-[50px] px-1 py-0.5 bg-slate-700/50 border border-slate-700/20 rounded-[3px] text-slate-200 text-xs text-right box-border focus:outline-none focus:border-blue-500 placeholder:text-slate-400 [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
        value={inputValue}
        step={opts.step}
        min={opts.min}
        max={opts.max}
...
export const SamTable: React.FC<Props> = ({
  ...
  const simulationCaption = simulationModeActive ? (
    <div className="px-8 py-4 bg-gradient-to-br from-[rgba(15,25,60,1)] to-[rgba(40,45,80,1)] text-blue-500 text-lg font-semibold tracking-wide text-center uppercase">
  ...
  const renderBasicTable = () => (
    <Table
      className="border-separate border-spacing-y-1 border-spacing-x-0 font-sans [&_thead_th]:font-medium [&_thead_th]:text-[11px] [&_thead_th]:uppercase [&_thead_th]:tracking-wide [&_thead_th]:text-slate-400 [&_thead_th]:px-4 [&_thead_th]:py-2 [&_tbody_tr]:transition-colors [&_tbody_td]:px-4 [&_tbody_td]:py-3 [&_tbody_td]:bg-slate-800 [&_tbody_td]:border-y [&_tbody_td]:border-slate-700/10 [&_tbody_td:first-child]:border-l-[3px] [&_tbody_td:first-child]:border-l-transparent [&_tbody_td:first-child]:rounded-l-lg [&_tbody_td:last-child]:border-r [&_tbody_td:last-child]:border-r-slate-700/10 [&_tbody_td:last-child]:rounded-r-lg [&_tbody_tr:hover_td]:bg-blue-500/10 [&_tbody_tr:hover_td:first-child]:border-l-blue-500"
          ...
          render: item => {
            ...
            return (
              ...
              >
                ...
                <span
                  className="text-[11px] text-slate-400 cursor-pointer hover:text-slate-200 hover:underline"
          ...
          <div className="relative inline-flex items-center gap-1">
            ...
            {isEditing && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 flex gap-1 z-10">
                <button
                  className={cn(
                    'min-w-[60px] px-2 py-[3px] bg-blue-500 text-white border-none rounded-[3px] cursor-pointer text-[10px] font-medium transition-colors whitespace-nowrap shadow-[0_1px_3px_rgba(0,0,0,0.3)] text-center hover:brightness-110 disabled:bg-slate-800/50 disabled:text-slate-400 disabled:cursor-not-allowed',
                    isCalculating && 'bg-slate-800/50 text-slate-400',
                  )}
                  onClick={e => {
                    e.stopPropagation()
                    onRunSimulation()
                ...
                <button
                  className="px-1.5 py-[3px] bg-slate-400 text-white border-none rounded-[3px] cursor-pointer text-[10px] font-medium transition-colors shadow-[0_1px_3px_rgba(0,0,0,0.3)] hover:enabled:bg-red-400"
                  onClick={e => {
                    e.stopPropagation()
                    onCancelEditing()
                  }}

...
import { selectName, selectTotalMarinadeStake, selectVoteAccount } from "src/services/validators";
    ...
    return <div className={styles.tableWrap}>
        <div className={styles.matricWrap}>
            ...
            <Metric label="Bonds balance" value={`☉ ${formatSolAmount(effectiveBalance)}`}
                data-tooltip-id="tooltip" data-tooltip-html="Total effective amount of SOL deposited to the bonds" />
            <Metric label="Marinade stake" value={`☉ ${formatSolAmount(totalMarinadeStake)}`}
                data-tooltip-id="tooltip" data-tooltip-html="How much stake is distributed by Marinade" />
            <Metric label="Protected stake" value={`☉ ${formatSolAmount(totalProtectedStake)}`}
                data-tooltip-id="tooltip" data-tooltip-html="How much of Marinade's stake is protected by validators' deposits to the bonds" />
        ...
        <Table
            ...
            columns={[
                ...
                { header: 'Bond balance [☉]', render: ({ bond }) => <>{formatSolAmount(Number(lamportsToSol(bond?.effective_amount ?? '0')))}</>, compare: (a, b) => Number(a.bond?.effective_amount ?? 0) - Number(b.bond?.effective_amount ?? 0), alignment: Alignment.RIGHT },
                { header: 'Marinade stake [☉]', render: ({ validator }) => <>{formatSolAmount(selectTotalMarinadeStake(validator))}</>, compare: (a, b) => selectTotalMarinadeStake(a.validator) - selectTotalMarinadeStake(b.validator), alignment: Alignment.RIGHT },
